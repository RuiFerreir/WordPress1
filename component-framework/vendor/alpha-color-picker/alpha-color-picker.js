...
( function( $ ) {
	 ...
	 */
	 function acp_update_alpha_value_on_color_input( alpha, $input, $alphaSlider, update_slider ) {
		...
		colorPicker = $input.data( 'ctCtColorPicker' );
		...
		$input.ctColorPicker( 'color', color ); // If we let Iris to change the color itself, it will change the square selector

		// Instead, we are changing Iris internal values ourselves
		// var newColor = new Color( color ).setHSpace( 'hsl' );
		// if ( !newColor.error ) {
		// 	iris._color = newColor;
		// 	iris.options.color = iris.options['color'] = iris._color.toString();
		// 	iris.active = 'strip'; // Here's the trick
		// 	iris._change();
		// }
	}
	...
	$.fn.alphaColorPicker = function(options) {
		...
		return this.each( function() {
			...
			// around the HTML added by ctColorPicker().
			...
			// Set up the options that we'll pass to ctColorPicker().
			colorPickerOptions = {
				change: function( event, ui ) {
					...
					value = $input.ctColorPicker( 'color' );
			...
			$input.ctColorPicker( colorPickerOptions );
			...
			$( '<div class="alpha-color-picker-container">' +
				...
				'</div>' ).css('width', '202px').appendTo( $container.find( '.wp-picker-holder' ) );
			...
			$container.find( '.iris-palette' ).on( 'click', function() {
				...
				$input.ctColorPicker( 'color', color );
			});
