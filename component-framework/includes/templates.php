...
function ct_template_output( $as_shortcodes = false ) {
	...
	if ( $shortcodes ) {
		...
		// parse [oxygen ] shortcodes in the urls and src
		...
		while(strpos($shortcodes, '[oxygen') !== false && $count < 9) {
			$count++;
			$shortcodes = preg_replace_callback('/(\")(url|src|map_address|alt|background-image)(\":\"[^\"]*)(\[oxygen[^\]]*\])([^\"\[\s]*)/i', 'ct_parse_oxy_url', $shortcodes);
		}
