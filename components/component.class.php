			...
			<?php if (isset($tab['tabs'])&&is_array($tab['tabs'])) {
				foreach ($tab['tabs'] as $child_id => $child_tab) { ?>
				    ...
				    <div class="oxygen-sidebar-flex-panel oxygen-basic-styles-tab-content"
				        ...
				        <?php $this->component_params($child_tab['params']);?>
		...
		if ( isset($params) && $params ) : 
			...
			foreach ( $params as $param ) : 
				...
				<div class='oxygen-control-wrapper <?php echo esc_attr($ct_class); ?>'
					...
					<div class='oxygen-control <?php if ( isset($param['css']) && $param['css'] === false ) echo "oxygen-special-property"; ?><?php if ( isset($param['responsive']) && $param['responsive'] === true ) echo " oxygen-responsive-property"; ?><?php if ( !in_array($param['param_name'], $options_white_list) && !in_array($param['param_name'], $options_white_list_no_media)) echo " not-available-for-classes"; ?><?php if ( !in_array($param['param_name'], $options_white_list)) echo " not-available-for-media"; ?>'>
						...
						<?php switch ( $param['type'] ) {
							...
							case 'tag' : ?>
								...
								<div class="oxygen-select oxygen-select-box-wrapper">
									...
									<div class="oxygen-select-box-options">
										<?php foreach ( $param['value'] as $value => $name ) : ?>
										<div class="oxygen-select-box-option" ng-click="iframeScope.setOptionModel('<?php echo esc_attr( $param['param_name'] ); ?>','<?php echo esc_attr( $value ); ?>');iframeScope.changeTag(<?php if ($this->options['tag']=='ct_shortcode') echo "'shortcode'"; else if (isset($this->options['data_type']) && $this->options['data_type']===true) echo "'data'"; ?>)">
											<?php echo sanitize_text_field( $name ); ?>
										</div>
										<?php endforeach; ?>
									</div>
							...
							case 'slider-measurebox' : ?>
								...
								<?php 
									...
									$oxygen_toolbar->slider_measure_box($param['param_name'],$param['param_units'],isset($param['min'])?$param['min']:0,isset($param['max'])?$param['max']:100, true, $param['step']); ?>
								<?php break;
							...
							case 'medialist_above' : ?>
								...
								<?php $oxygen_toolbar->media_queries_list($param['param_name'],$param['heading'], true); ?>
								<?php break;
			...
			foreach ( $this->options['tabs'] as $key => $tab ) {
				...
				if ( isset($tab['tabs']) && is_array($tab['tabs'])) {
					foreach ( $tab['tabs'] as $key => $child_tab ) {
						...
						$tabs_params = array_merge( 
											$tabs_params,
											$this->get_default_params_helper($child_tab['params'], $not_css)
										);
	...
	function build_css($states, $selector, $is_media = false) {
		...
		foreach ( $states as $key => $atts ) {
			...
			if (($this->options['tag'] == "oxy_nav_menu") && 
				 $states['original']["menu_color"]) {
				$styles .= '#' . $selector . " .oxy-nav-menu-hamburger-line{";
				$styles .= "background-color:" . $states['original']["menu_color"] . ";";
				$styles .= "}";
			}

			// make open menu bg same as header/row bg
			if (($this->options['tag'] == "oxy_header" || $this->options['tag'] == "oxy_header_row") && $states['original']["background-color"]) {
				$styles .= '#' . $selector . " .oxy-nav-menu-open,";
				$styles .= '#' . $selector . " .oxy-nav-menu:not(.oxy-nav-menu-open) .sub-menu{";
				$styles .= "background-color:".$states['original']["background-color"].";";
				$styles .= "}";
			}
			...
			if ( $this->options['tag'] == "oxy_nav_menu" ) {
				...
				if (isset($states['original']['menu_hover_background-color'])&&!$states['original']['menu_dropdowns_background-color']) {
					$oxy_nav_menu_styles_dropdowns = "background-color:" . $states['original']['menu_hover_background-color'] . ";";
				}
				$oxy_nav_menu_styles_dropdowns_items = "border: 0;";

				if ($states['original']['menu_justify-content']) {
					$oxy_nav_menu_styles_not_open_items = "justify-content: " . $states['original']['menu_justify-content'];
				}

				if ($states['original']['menu_flex-direction']=="column") {
					...
					$oxy_nav_menu_styles_dropdowns_items 	.= "padding-top: " . $states['original']['menu_padding-top'] . "px;";
					$oxy_nav_menu_styles_dropdowns_items 	.= "padding-bottom: " . $states['original']['menu_padding-bottom'] . "px;";
				}

				if ($atts['menu-responsive']!='never') {
				...
				$menuWidth 		= ($states['original']['menu_responsive_icon_size']) ? intval($states['original']['menu_responsive_icon_size']) : 40;
				$menuPadding 	= ($states['original']['menu_responsive_padding_size']) ? intval($states['original']['menu_responsive_padding_size']) : 0;
				...
				if ( $key=="original" && !$is_media ) {
					...
					$padding_top = (($states['original']['menu_responsive_padding_top']) ? $states['original']['menu_responsive_padding_top'] : $states['original']['menu_padding-top']);
					$padding_bottom = (($states['original']['menu_responsive_padding_bottom']) ? $states['original']['menu_responsive_padding_bottom'] : $states['original']['menu_padding-bottom']);
					$padding_left = (($states['original']['menu_responsive_padding_left']) ? $states['original']['menu_responsive_padding_left'] : $states['original']['menu_padding-left']);
					$padding_right = (($states['original']['menu_responsive_padding_right']) ? $states['original']['menu_responsive_padding_right'] : $states['original']['menu_padding-right']);

				$styles 
					.= '#' . $selector . ".oxy-nav-menu.oxy-nav-menu-open {" .
						"background-color:" . $states['original']['menu_responsive_background_color'] . ";" .
					...
					if (isset($states['original']['menu_responsive_link_color'])&&$states['original']['menu_responsive_link_color']!=="") { 
						$styles .=  "color:" . $states['original']['menu_responsive_link_color'] . ";";
					}
				...
				$styles 
					.='} #' . $selector . ".oxy-nav-menu.oxy-nav-menu-open .menu-item a:hover {" .
						"color:"  			. $states['original']['menu_responsive_hover_link_color'] . ";";

				$styles 
					.='} #' . $selector . ".oxy-nav-menu .menu-item a:hover {" .
					...
					'} #' . $selector . " .oxy-nav-menu-hamburger-wrap {".
						...
						"margin-top:"	 	. $states['original']['menu_responsive_icon_margin'] . "px;" .
						"margin-bottom:"	. $states['original']['menu_responsive_icon_margin'] . "px;" .
						"background-color:" . $states['original']['menu_responsive_padding_color'] . ";" .
					...
					'} #' . $selector . " .oxy-nav-menu-hamburger-line {" .
						...
						"background-color:" . $states['original']['menu_responsive_icon_color'] . ";" .
			...
			foreach ( $atts as $param => $value ) {
				...
				if ( isset($default_atts[$default_param.'-unit']) && $default_atts[$default_param.'-unit'] ) {
                    ...
                    if ($param == 'container-padding-top'||
                        ...
                        $unit = ( $atts[$param.'-unit'] ) ? $atts[$param.'-unit'] : $global_settings['sections'][$param.'-unit'];
                        if ( $atts[$param] ) {
                            $atts[$param] .= $unit;
                        }
                    }
			...
			if ( $this->options['tag'] == "oxy_header" ) {
				...
				if ( isset($atts['sticky-header']) && isset($atts['sticky-media']) && 
					...
					$stickyStyles .= "@media (min-width: ".$media_queries_list_above[$atts['sticky-media']]['minSize'].") {";
						...
						$stickyStyles .= "position: fixed; top: 0; left: 0; right: 0; z-index: 2147483640;";
						$stickyStyles .= ($atts['sticky-background-color']) ? "background-color:" . $atts['sticky-background-color'] . ";" : "";
						if ($atts['sticky-box-shadow']) {
			...
			if ( $this->options['tag'] == "ct_slider" ) {

				if ( $atts['slider-arrow-color'] && 
					 $atts['slider-arrow-color'] == 'lighter') {

					$styles .= '#' . $selector . " .unslider-arrow {";
					$styles .= "background-color: rgba(255,255,255,0.2); ";
					$styles .= "}";
				}

				if ( $atts['slider-dot-color'] ) {

					$styles .= '#' . $selector . " .unslider-nav ol li {";
					$styles .= "border-color: " . $atts['slider-dot-color'] . "; ";
					$styles .= "}";

					$styles .= '#' . $selector . " .unslider-nav ol li.unslider-active {";
					$styles .= "background-color: " . $atts['slider-dot-color'] . "; ";
					$styles .= "}";
				}
			}
			...
			if ($this->options['tag'] == "ct_link_button") {
				...
				if (!$is_media && $key == "original") {
					...
					$buttonColor 		= $this->states[$key]['button-color'];
					$buttonTextColor 	= $this->states[$key]['button-text-color'];
					...
					$buttonStyle 		= $this->css_states['original']['button-style'];
					$buttonColor 		= $this->states['media'][$is_media][$key]['button-color'];
					$buttonTextColor 	= $this->states['media'][$is_media][$key]['button-text-color'];
			...
			if ($this->options['tag'] == "ct_fancy_icon") {
				...
				if (!$is_media && $key == "original") {
					...
					$iconColor 			= $this->states[$key]['icon-color'];
					$iconBackgroundColor= $atts['icon-background-color'];
					$iconPadding 		= $atts['icon-padding'];
					$iconSize 			= ($this->states[$key]['icon-size']) ? $this->states[$key]['icon-size']."px" : "";
				}
				else {
					$iconStyle 			= $this->css_states['original']['icon-style'];
					$iconColor 			= $this->states['media'][$is_media][$key]['icon-color'];
					$iconBackgroundColor= $this->states['media'][$is_media][$key]['icon-background-color'];
					$iconPadding 		= ($this->states['media'][$is_media][$key]['icon-padding']) ? $this->states['media'][$is_media][$key]['icon-padding']."px" : "";
					$iconSize 			= ($this->states['media'][$is_media][$key]['icon-size']) ? $this->states['media'][$is_media][$key]['icon-size']."px" : "";
				}
			...
			if ( $this->options['tag'] == "ct_section" ) {
				...
				if ( isset($atts['video-background-overlay']) ) {
					$styles .= '#' . $selector . " .oxy-video-overlay { background-color: " . $atts['video-background-overlay'] . "; }";
				}
			...
			if ( isset($atts['box-shadow-color']) ) {
				...
				$inset 	= ($atts['box-shadow-inset']=='inset') 				? $atts['box-shadow-inset']." " : "";
				...
				$atts['box-shadow'] = $inset.$hor.$ver.$blur.$spread.$atts['box-shadow-color'];
			}
			...
			if ( isset($atts['text-shadow-color']) ) {
				...
				$atts['text-shadow'] = $hor.$ver.$blur.$atts['text-shadow-color'];
			}
				...
				if ( $prop != "custom-css" && $prop != "background-layers") {
					...
					if(!is_array($value)) {
						...
						// handle "menu_..." options that applies to menu items
							...
							if ( is_array($not_css_options[$this->options['tag']]) && in_array( str_replace("-", "_", $prop), $not_css_options[$this->options['tag']] ) ) {
								// do nothing
							}
							else {
			...
			if ( $this->options['tag'] == "oxy_nav_menu" ) {
				$styles .= ( $oxy_nav_menu_styles ) 				? $oxy_nav_menu_selector 				. $oxy_nav_menu_styles . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_item ) 			? $oxy_nav_menu_selector_item   		. $oxy_nav_menu_styles_item . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_active ) 			? $oxy_nav_menu_selector_active 		. $oxy_nav_menu_styles_active . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_dropdowns ) 		? $oxy_nav_menu_selector_dropdowns 		. $oxy_nav_menu_styles_dropdowns . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_not_open_items ) ? $oxy_nav_menu_selector_not_open_items . $oxy_nav_menu_styles_not_open_items . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_dropdowns_items ) ? $oxy_nav_menu_selector_dropdowns_items . $oxy_nav_menu_styles_dropdowns_items . "}\r\n" : "";
				$styles .= ( $oxy_nav_menu_styles_dropdowns_items_hover ) ? $oxy_nav_menu_selector_dropdowns_items_hover . $oxy_nav_menu_styles_dropdowns_items_hover . "}\r\n" : "";
			}
		}
